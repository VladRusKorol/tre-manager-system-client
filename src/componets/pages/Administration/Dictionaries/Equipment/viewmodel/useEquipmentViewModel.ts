...
const initEventDataGridRef: TEventDataGridRef<IEquipment> = {
    event: "new",
    popupTitle: "СОЗДАНИЕ НОВОГО ТРАНСПОРТА В СИСТЕМЕ УЧЕТА КГШ",
    popupTitleEditName: null
}

export const useEquipmentViewModel = () => {

    const eventDataGridRef = useRef<TEventDataGridRef<IEquipment>>({...initEventDataGridRef})

    const dgRef = useRef<DataGrid<IEquipment, number> | null>(null);
     ...
     */
    const onInitNewRowCallback = useCallback((e: EventInfo<dxDataGrid<IEquipment, number>> & NewRowInfo<IEquipment>)=>{
     ...
     */
    const onEditingStartCallback = useCallback((e: EditingStartEvent<IEquipment, number>)=> {
        eventDataGridRef.current = {
            ...eventDataGridRef.current,
            event: "edit",
            popupTitle: "Редактирование транспорта",
