...
const initEventDataGridRef: TEventDataGridRef<ITireModel> = {
    event: "new",
    popupTitle: "СОЗДАНИЕ НОВОЙ МОДЕЛИ ШИНЫ В СИСТЕМЕ УЧЕТА КГШ",
    popupTitleEditName: null
}

export const useTireModelViewModel = () => {

    const eventDataGridRef = useRef<TEventDataGridRef<ITireModel>>({...initEventDataGridRef})

    const dgRef = useRef<DataGrid<ITireModel, number> | null>(null);
     ...
     */
    const onInitNewRowCallback = useCallback((e: EventInfo<dxDataGrid<ITireModel, number>> & NewRowInfo<ITireModel>)=>{
     ...
     */
    const onEditingStartCallback = useCallback((e: EditingStartEvent<ITireModel, number>)=> {
        eventDataGridRef.current = {
            ...eventDataGridRef.current,
            event: "edit",
            popupTitle: "Редактирование модели шины".toLocaleUpperCase(),
