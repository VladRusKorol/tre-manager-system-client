...
interface IProps {
    dataSource: DataSource<ITireSeasonalityType, number>,
    dgRef: RefObject<DataGrid<ITireSeasonalityType, number> | null>;
    
    onEditingCancelCallback: () => void;
    onInitNewRowCallback: (e: EventInfo<dxDataGrid<ITireSeasonalityType, number>> & NewRowInfo<ITireSeasonalityType>) => void;
    onRowInsertedCallback: (e: RowInsertedEvent<ITireSeasonalityType, number>) => void;
    onEditingStartCallback: (e: EditingStartEvent<ITireSeasonalityType, number>) => void;
    onTitleRenderCallback: () => JSX.Element | undefined
    onFormPopupShownCallback: (e: any) => void
    //onFilterSwitchCallback: () => void
}

export const TireSeasonalityTypeGrid: React.FC<IProps> = React.memo(({
    dataSource,
    dgRef,
    ...
    return<>
        <DataGrid<ITireSeasonalityType,number>
            ref={dgRef}
        ...
        >
            ...
            <Column 
                dataField="idTireSeasonalityType" 
                caption="№" 
                width={70} 
                alignment="center" 
                sortIndex={0} 
            ...
            <Column type={"buttons"} caption={"ДЕЙСТВИЯ"} width={"120px"}>
                ...
                <Button 
                    ...
                    icon="trash" 
                    text="УДАЛИТЬ" 
                    visible={ (e: any) => e.row.data && (e.row.data as ITireSeasonalityType).tireModels.length ===0 }
                />
            </Column>
